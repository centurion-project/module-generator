...
foreach ($elements as $element) {
    $body .= '\'' . $element . '\'              => $this->_translate(\'' . ucfirst(str_replace(array('_id', '_at') , '', $element)) .'\'),' . "\n";
}
...
EOS;
    ...
    protected function _makeFrontViews($moduleName, $object, $table)
    {
        $viewName = str_replace('_', '-', Centurion_Inflector::tableize($object));
        $viewPath = APPLICATION_PATH . DIRECTORY_SEPARATOR . 'modules' . DIRECTORY_SEPARATOR . strtolower($moduleName) . DIRECTORY_SEPARATOR . 'views' . DIRECTORY_SEPARATOR . 'scripts' . DIRECTORY_SEPARATOR . $viewName;
        ...
        $scriptsToCreate = array('list.phtml', 'get.phtml');
        
        foreach ($scriptsToCreate as $scriptName) {
            $viewScript = $viewPath . DIRECTORY_SEPARATOR . $scriptName;
            file_put_contents($viewScript, '');
        }
    }
...
EOS;
    if ($firstcol !== null) {
        $ucFirstcol = ucfirst($firstcol);
        $body .= <<<EOS
'$firstcol' => array('type' => Centurion_Controller_CRUD::COL_TYPE_FIRSTCOL,
                 'param' => array('cover' => null, 'title' => '$firstcol', 'subtitle' => null),
                 'label' => \$this->view->translate('$ucFirstcol')),

EOS;
}
        foreach ($displays as $display) {
            $ucDisplay = str_replace(array('_id', '_at'), '', ucfirst($display));
			
$body .= <<<EOS
    '$display' => \$this->view->translate('$ucDisplay'),
...
EOS;
    ...
    public function _makeFrontController($modules, $tables)
    {
        foreach ($tables as $table) {
            ...
            $this->_makeFrontViews($moduleName, $className, $table);
            
            $oldClassName = lcfirst($className);
            ...
            if ($class == null) {
                $class = new Zend_CodeGenerator_Php_Class();
                $class->setName($moduleName . '_'.$className);
                $class->setExtendedClass('Centurion_Controller_Action');
                
                $class->setProperty(array('visibility' => 'protected', 'name' => '_model', 'value' => $moduleName . '/'.$className));
            }

            $method = $class->getMethod('indexAction');
            if (false === $method) {
                $method = new Zend_CodeGenerator_Php_Method();
...
EOS
            ...
            $method = $class->getMethod('listAction');
            if (false === $method) {
                $method = new Zend_CodeGenerator_Php_Method();
                $method->setName('listAction');

                $class->setMethod($method);
            }

            $method->setBody(<<<EOS
\$this->view->{$oldClassName}RowSet = Centurion_Db::getSingleton('$lcModuleName/$oldClassName')->select(true)
                                                                ->fetchAll();
EOS
            );
            ...
            if (false === $method) {
                $method = new Zend_CodeGenerator_Php_Method();
                $method->setName('getAction');

                $class->setMethod($method);
            }

            $method->setBody(<<<EOS
\$this->view->{$oldClassName}Row = \$this->_helper->getObjectOr404('$lcModuleName/$oldClassName', array('id' => \$this->_getParam('id')));
EOS
            );
